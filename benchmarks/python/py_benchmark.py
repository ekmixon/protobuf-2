...
if args.cpp_generated != "no":
  sys.path.append(
      f"{os.path.dirname(os.path.dirname( os.path.abspath(__file__) ))}/.libs")
  import libbenchmark_messages
  sys.path.append(
      f"{os.path.dirname(os.path.dirname( os.path.abspath(__file__) ))}/tmp")
# END CPP GENERATED MESSAGE
...
def init(filename):
  ...
  if benchmark_dataset.message_name == "benchmarks.proto3.GoogleMessage1":
    ...
    raise IOError(f"Message {benchmark_dataset.message_name} not found!")
...
class Benchmark:
  ...
  def dry_run(self, test_method_args='', setup_method_args=''):
    return timeit.timeit(
        stmt=f"{self.test_method}({test_method_args})",
...
if __name__ == "__main__":
  ...
  if args.json != "no":
    ...
    for result in results:
      print(f'Message {result["message_name"]} of dataset file {result["filename"]}')
      print(
